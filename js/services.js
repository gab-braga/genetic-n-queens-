import { copyQueens, generateQueens, randInt } from "./utils.js";
...
function calculateFitness(queens, size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      ...
      if (diagonal || vertical || horizontal) collisions++;
    }
  }
  return collisions;
}
...
function select(population, boardSize) {
  ...
  for (let i = 0; i < size; i++) {
    let choice = randInt(1, totalFactor);
    for (let j = 0; j < size; j++) {
      const { queens, fitness, factor } = roulette[j];
      if (choice <= factor) {
        selection.push({
          queens: copyQueens(queens),
          fitness,
        });
        break;
      } else choice -= factor;
...
function mutate(population, boardSize) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    geration.push({ queens, fitness: fitness });
  }
  return geration;
}

export { generatePopulation, getSolution, select, crossover, mutate };
